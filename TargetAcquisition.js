var TargetAcquisition = function () {
   ...
   this.platformArtist = {
      draw: function (sprite, context) {
         ...
         top = targetAcquisition.calculatePlatformTop(sprite.track);
   ...
   this.paceBehavior = {
      setDirection: function (sprite) {
         ...
         if (sprite.direction === undefined) {
            sprite.direction = targetAcquisition.RIGHT;
         }

         if (sRight > pRight && sprite.direction === targetAcquisition.RIGHT) {
            sprite.direction = targetAcquisition.LEFT;
         }
      ...
      setPosition: function (sprite, now, lastAnimationFrameTime) {
         ...
         if (sprite.direction === targetAcquisition.RIGHT) {
   ...
   this.snailShootBehavior = { // sprite is the snail
      execute: function (sprite, now, fps, context, 
         ...
         if ( ! targetAcquisition.isSpriteInView(sprite)) {
            return;
         }
   ...
   this.jumpBehavior = {
      ...
      finishDescent: function (sprite, now) {
         sprite.stopJumping();
         sprite.runAnimationRate = targetAcquisition.RUN_ANIMATION_RATE;

         if (targetAcquisition.platformUnderneath(sprite)) {
   ...
   this.collideBehavior = {
      ...
      processPlatformCollisionDuringJump: function (sprite, platform) {
         if (sprite.descendTimer.isRunning()) {
            ...
            sprite.top = targetAcquisition.calculatePlatformTop(sprite.track) - 
                         sprite.height;

            sprite.descendTimer.stop(
               targetAcquisition.timeSystem.calculateGameTime());
      ...
      execute: function (sprite, now, fps, context, 
         ...
         for (var i=0; i < targetAcquisition.sprites.length; ++i) {
            otherSprite = targetAcquisition.sprites[i];
...
TargetAcquisition.prototype = {
   ...
   equipRunnerForJumping: function () {
      ...
      this.runner.jump = function () {
         ...
         this.ascendTimer.start(targetAcquisition.timeSystem.calculateGameTime());
      };

      this.runner.stopJumping = function () {
         this.ascendTimer.stop(targetAcquisition.timeSystem.calculateGameTime());
         this.descendTimer.stop(targetAcquisition.timeSystem.calculateGameTime());
         this.jumping = false;
      };

      this.runner.fall = function () {
         // For now...
         targetAcquisition.runner.track = 1;
         targetAcquisition.runner.top = targetAcquisition.calculatePlatformTop(targetAcquisition.runner.track) -
                             targetAcquisition.runner.height;
      };
   },
   ...
   setTimeRate: function (rate) {
      ...
      this.timeSystem.setTransducer( function (percent) {
         return percent * targetAcquisition.timeFactor;
      });      
   },
   ...
   createSnailSprites: function () {
      ...
      for (var i = 0; i < this.snailData.length; ++i) {
         ...
         snail.velocityX = targetAcquisition.SNAIL_PACE_VELOCITY;
   ...
   armSnails: function () {
      ...
      for (var i=0; i < this.snails.length; ++i) {
         ...
         snail.bomb.width  = targetAcquisition.SNAIL_BOMB_CELLS_WIDTH;
         snail.bomb.height = targetAcquisition.SNAIL_BOMB_CELLS_HEIGHT;
   ...
   */
   ...
   platformUnderneath: function (sprite, track) {
      ...
      for (var i=0; i < targetAcquisition.platforms.length; ++i) {
         platform = targetAcquisition.platforms[i];
   ...
   fadeInElements: function () {
      ...
      setTimeout( function () {
         for (var i=0; i < args.length; ++i) {
            args[i].style.opacity = targetAcquisition.OPAQUE;
         }
      }, this.SHORT_DELAY);
   },
   ...
   initializeImages: function () {
      ...
      this.spritesheet.onload = function (e) {
         targetAcquisition.backgroundLoaded();
      };

      this.runnerAnimatedGIFElement.onload = function () {
         targetAcquisition.loadingAnimationLoaded();
      };
   },
   ...
   dimControls: function () {
      ...
      targetAcquisition.instructionsElement.style.opacity = FINAL_OPACITY;
      targetAcquisition.soundAndMusicElement.style.opacity = FINAL_OPACITY;
   },
   ...
   revealTopChromeDimmed: function () {
      ...
      setTimeout( function () {
         targetAcquisition.scoreElement.style.opacity = DIM;
         targetAcquisition.fpsElement.style.opacity = DIM;
      }, this.SHORT_DELAY);
   },
   ...
   revealGame: function () {
      ...
      setTimeout( function () {
         targetAcquisition.dimControls();
         targetAcquisition.revealTopChrome();
      }, DIM_CONTROLS_DELAY);
   },   
   ...
   revealInitialToast: function () {
      ...
      setTimeout( function () {
         targetAcquisition.revealToast('Solve the puzzle. ' + 'Level One', 
...
window.onkeydown = function (e) {
   ...
   if (key === 68 || key === 37) { // 'd' or left arrow
      targetAcquisition.turnLeft();
   }
   else if (key === 75 || key === 39) { // 'k' or right arrow
      targetAcquisition.turnRight();
   }
   else if (key === 80) { // 'p'
      targetAcquisition.togglePaused();
   }
   else if (key === 74) { // 'j'
      targetAcquisition.runner.jump();
   }
};

window.onblur = function (e) {  // pause if unpaused
   targetAcquisition.windowHasFocus = false;
...
window.onfocus = function (e) {  // unpause if paused
   var originalFont = targetAcquisition.toastElement.style.fontSize,
       DIGIT_DISPLAY_DURATION = 1000; // milliseconds

       targetAcquisition.windowHasFocus = true;
       targetAcquisition.countdownInProgress = true;

       if (targetAcquisition.paused) {
         targetAcquisition.toastElement.style.font = '128px fantasy'; // Large font

      if (targetAcquisition.windowHasFocus && targetAcquisition.countdownInProgress)
         targetAcquisition.revealToast('3', 500); // Display 3 for 0.5 seconds

      setTimeout(function (e) {
         if (targetAcquisition.windowHasFocus && targetAcquisition.countdownInProgress)
            targetAcquisition.revealToast('2', 500); // Display 2 for 0.5 seconds

         setTimeout(function (e) {
            if (targetAcquisition.windowHasFocus && targetAcquisition.countdownInProgress)
               targetAcquisition.revealToast('1', 500); // Display 1 for 0.5 seconds
            ...
            setTimeout(function (e) {
               ...
               if ( targetAcquisition.windowHasFocus && targetAcquisition.countdownInProgress)
                  targetAcquisition.toastElement.style.fontSize = originalFont;

               targetAcquisition.countdownInProgress = false;
...
targetAcquisition.createSprites();
